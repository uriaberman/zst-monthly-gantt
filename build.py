...
/* DELETE BUTTON — destructive action, lives next to modal-close in Editor only.
   Iron Rule #20 (25.5.26): Every item modal MUST expose a permanent-delete trash icon.
   This is different from status "נגנז" (archive/mute) — delete REMOVES the item from data.json.
   See memory: feedback_gantt_delete_button. */
.modal-delete {
...
body.theme-uria .modal-delete {
  background: rgba(239,68,68,0.14);
  border-color: rgba(239,68,68,0.50);
  color: #FCA5A5;
}
body.theme-uria .modal-delete:hover {
  background: #EF4444;
  color: #FAFAFA;
}

/* PENDING-DELETE state for cell-item (set by applyDeletionsToCells before publish).
   Iron Rule #20 (סבב ג, 25.5.26): מחיקה באדיטור = הפריט נעלם מיידית מהגריד.
   ה-Draft Bar + deletions-panel מתחתיו מציגים את הסטטוס + אפשרות לבטל ספציפית. */
.cell-item.is-pending-delete {
  display: none !important;
}

/* DELETIONS PANEL — sits right below the Draft Bar, lists items marked for deletion
   with per-item restore buttons. Hidden when there are no pending deletions. */
.deletions-panel {
  display: flex;
  flex-wrap: wrap;
  align-items: center;
...
.deletions-panel[hidden] { display: none !important; }
.deletions-panel-title {
  font-weight: 800;
  color: #EF4444;
  display: inline-flex;
  align-items: center;
...
.deletions-panel-title svg { display: block; }
.deletions-list {
  display: inline-flex;
  flex-wrap: wrap;
  gap: 6px;
...
.deletions-list-item .item-title {
  color: var(--ink);
  font-size: 12px;
  font-weight: 600;
  white-space: nowrap;
...
body.theme-uria .deletions-panel { background: rgba(239,68,68,0.10); border-color: rgba(239,68,68,0.40); color: #FCA5A5; }
...
/* ============================================================
   📱 RESPONSIVE — MOBILE FIRST (v9.9.1, 25.5.26)
   Critical for the Viewer URL: clients open it from WhatsApp on their phones.
   Strategy:
   - Tablet (≤ 1024px): tighter spacing, smaller fonts, still 7-col grid
   - Mobile (≤ 640px): stack the calendar — one cell per row, weekdays hidden,
     empty cells (no items) hidden, modal full-screen
   - Small mobile (≤ 380px): even more compact paddings/fonts
   ============================================================ */

/* TABLET (≤ 1024px) — keep grid, tighten everything */
@media (max-width: 1024px) {
  .container { padding: 16px 14px; }
  .header-titles h1 { font-size: 22px; }
  .header-titles .period { font-size: 13px; }
  .cell { min-height: 130px; padding: 8px; }
  .cell-title { font-size: 12.5px; }
  .controls { gap: 10px; }
  .legend { gap: 6px 10px; padding: 6px 12px; font-size: 11px; }
...
@media (max-width: 640px) {
  ...
  /* HEADER — compact, stack the titles */
  .header {
    flex-direction: column;
    align-items: flex-start;
    gap: 10px;
  ...
  .header-titles h1 { font-size: 19px; line-height: 1.2; }
  .header-titles .period { font-size: 12px; }
  .logo-mark, .uria-x-logo { transform: scale(0.85); transform-origin: right center; }

  /* CONTROLS — stack legend + cluster vertically */
  .controls {
    flex-direction: column;
    align-items: stretch;
    gap: 10px;
  ...
  .month-picker { flex: 1; min-width: 0; justify-content: center; }
  .month-dd-trigger { min-width: 0; flex: 1; }
  .month-dd-panel { right: auto; left: 0; min-width: 140px; }

  /* WEEKDAYS HEADER — hide on mobile (each cell shows day name inline) */
  .weekdays { display: none; }

  /* GRID — one column. Each .week becomes a flat stack. */
  .week {
    display: flex;
    flex-direction: column;
    gap: 8px;
  ...
  /* CELLS — hide empty days (outside / friday / saturday without content);
     show only cells with content OR the current day (so user knows where "today" is). */
  .cell.outside,
  .cell.friday:not(.has-content),
  .cell.saturday:not(.has-content) {
    display: none;
  }
  /* Empty weekday cells without content also hidden — declutter the mobile view */
  .cell:not(.has-content):not(.is-today) { display: none; }

  /* Cells that ARE shown: full width, taller for touch */
  .cell {
    min-height: auto;
    padding: 12px 14px;
    border-radius: 12px;
  ...
  .cell-num-row, .cell-heb { font-size: 11.5px; }
  .cell-title { font-size: 14px; line-height: 1.35; }
  .cell-type-row { font-size: 11px; }
  .cell-status-pill { font-size: 11px; padding: 3px 9px; }
  .cell-open { font-size: 11.5px; padding: 5px 10px; }
  ...
  .modal-head-left h2 { font-size: 19px; }
  .modal-head-left .date-row { font-size: 11.5px; gap: 6px; }
  .modal-body { padding: 14px 16px 24px; }
  .modal-primary { grid-template-columns: 1fr; gap: 14px; }
  .copy-area textarea, .copy-view {
    font-size: 14px;
    min-height: 180px;
  }
  .modal-pill { font-size: 10.5px; }
  .modal-publish-label { font-size: 10px; }
  .modal-close, .modal-delete { width: 32px; height: 32px; }

  /* MEDIA GRID inside modal: 2 columns on mobile (already handled above but tighten) */
  .media-grid { grid-template-columns: repeat(2, 1fr); gap: 8px; }
  .media-zone-label { font-size: 11px; padding: 6px 10px; }

  /* DRAFT BAR — stack on mobile, larger touch targets */
  .draft-bar {
    flex-direction: column;
    align-items: stretch;
    gap: 8px;
  ...
  .draft-bar-actions { justify-content: stretch; }
  .draft-bar-publish, .draft-bar-discard { flex: 1; padding: 10px 12px; font-size: 12px; }

  /* DELETIONS PANEL — stack chips on mobile */
  .deletions-panel { flex-direction: column; align-items: stretch; padding: 10px 12px; }
  .deletions-list { flex-direction: column; align-items: stretch; gap: 6px; }
  .deletions-list-item { max-width: 100%; justify-content: space-between; }

  /* FOOTER — compact */
  .footer-meta { flex-direction: column; align-items: stretch; gap: 8px; text-align: center; }
  .footer-link { font-size: 11px; }

  /* PREVIEW TOGGLE (Editor only) — smaller on mobile */
  .preview-toggle {
    bottom: 14px;
    left: 14px;
    padding: 8px 12px;
...
/* SMALL MOBILE (≤ 380px) — even tighter for iPhone SE etc. */
...
@media (hover: none) {
  ...
  .month-dd-trigger:hover { color: var(--ink); }
}
'''
...
window.requestPublish = async function() {
  ...
  const hasItemEdits = Object.keys(draftChanges.items).length > 0;
  const hasDeletions = (draftChanges.deletions || []).length > 0;
  if (!hasItemEdits && !hasDeletions) {
  ...
  try {
    ...
    // 4. Apply local drafts (edits + deletions)
    const newData = mergeChangesIntoData(currentData, draftChanges);
    ...
    const editsSummary = Object.entries(draftChanges.items)
      .map(([num, d]) => `#${num}: ${Object.keys(d).join('+')}`)
      .join(', ');
    const delsSummary = (draftChanges.deletions || []).length > 0
      ? `delete #${(draftChanges.deletions || []).join(', #')}`
      : '';
    const summary = [editsSummary, delsSummary].filter(Boolean).join(' | ');
...
/* Clear local drafts without confirmation (called after successful publish).
   Also clears pending deletions (Iron Rule #20) since they're now applied server-side. */
...
/* Collect all unpublished changes from localStorage into a structured payload.
   Includes pending deletions (Iron Rule #20): items the user clicked the trash icon on.
   Deletions are filtered server-side in publish_cli.py — until then they're reversible. */
function collectDraftChanges() {
  const items = {};
  const dels = new Set(getDeletions());
  ITEMS_DATA.forEach(it => {
    const num = it.num;
    // Don't collect field-level edits for items already marked for deletion -
    // they'd be lost in the merge anyway (cleaner payload, easier server-side reasoning).
    if (dels.has(String(num))) return;
    const draft = {};
  ...
  return { items: items, deletions: Array.from(dels) };
}

/* Show the draft bar whenever there are local changes (Editor only).
   Counts edits AND pending deletions (Iron Rule #20). Also refreshes the deletions panel
   below it (per-item restore chips). */
function updateDraftBar() {
  if (VIEW_MODE_BAKED) return;  // viewer never shows the bar
  const bar = document.getElementById('draftBar');
  if (!bar) return;
  const ch = collectDraftChanges();
  const editCount = Object.keys(ch.items).length;
  const delCount = (ch.deletions || []).length;
  const total = editCount + delCount;
  bar.hidden = total === 0;
  const text = bar.querySelector('.draft-bar-text');
  if (text && total > 0) {
    const parts = [];
    if (editCount > 0) parts.push(`${editCount} שינוי${editCount > 1 ? 'ים' : ''}`);
    if (delCount > 0)  parts.push(`${delCount} פריט${delCount > 1 ? 'ים' : ''} למחיקה`);
    text.textContent = `⚠ יש לך בטיוטה: ${parts.join(' + ')} — הלקוח עדיין לא רואה אותם`;
  }
  // Keep the deletions panel in sync — shows per-item restore chips below the draft bar.
  try { updateDeletionsPanel(); } catch (e) {}
}
...
function openModal(num) {
  ...
  inner.innerHTML = `
    <div class="modal-head">
      ...
      <div class="modal-head-right">
        ${isView ? '' : `
          <button class="modal-delete" id="modalDelete" data-num="${num}" type="button"
                  title="מחק לצמיתות (נדרש פרסום לסיום)" aria-label="מחק פריט זה לצמיתות">
  ...
  // Wire up the permanent-delete trash button (Iron Rule #20 — see SKILL.md).
  // Soft-delete pattern: marks the item in localStorage as pending-delete, shows it
  // dimmed+strikethrough on the grid, and waits for "publish" to actually remove from data.json.
...
/* ---------- Permanent delete flow (Iron Rule #20, 25.5.26) ----------
   STATE: list of item nums stored in localStorage['gantt:deletions:'+CLIENT_KEY] (JSON array).
   UI:    cell-items in the list get .is-pending-delete (CSS strikethrough + red badge).
   PUBLISH: collectDraftChanges() includes deletions array; publish_cli.py filters them out
           of data.json server-side. Until publish, the deletion is reversible via "בטל טיוטה".
*/
const DELETIONS_LS_KEY = 'gantt:deletions:' + CLIENT_KEY;

function getDeletions() {
  try {
    const raw = localStorage.getItem(DELETIONS_LS_KEY);
    if (!raw) return [];
...
function saveDeletions(arr) {
  try { localStorage.setItem(DELETIONS_LS_KEY, JSON.stringify(arr.map(String))); } catch(e) {}
}

function clearDeletions() {
  try { localStorage.removeItem(DELETIONS_LS_KEY); } catch(e) {}
}
...
window.requestDelete = function(num) {
  const it = itemsByNum[num];
  if (!it) return;
  const dels = getDeletions();
  if (dels.includes(String(num))) {
    // Already marked — silently close (user can restore from the deletions panel)
    closeModal();
    return;
  }
  const msg =
  ...
  saveDeletions(dels);
  applyDeletionsToCells();
  updateDraftBar();
  closeModal();
};

/* Restore a single soft-deleted item — used by the per-item undo buttons in the
   deletions panel. Removes the num from localStorage deletions, refreshes the grid
   (cell-item becomes visible again), updates the draft bar. */
window.restoreDeletion = function(num) {
  const remaining = getDeletions().filter(d => String(d) !== String(num));
  saveDeletions(remaining);
  applyDeletionsToCells();
  updateDraftBar();
};

/* Draw the deletions panel below the Draft Bar: one chip per pending deletion,
   with item title + restore button. Hidden when there are zero deletions. */
function updateDeletionsPanel() {
  if (VIEW_MODE_BAKED) return;
  const panel = document.getElementById('deletionsPanel');
  ...
  // Render: for each pending deletion num, find the item and add a chip
  const html = dels.map(numStr => {
    const it = itemsByNum[numStr];
    if (!it) return '';
    const title = (it.title || '(ללא כותרת)').slice(0, 60);
    return `
      <li class="deletions-list-item">
        <span class="item-num">#${escapeHtml(String(numStr))}</span>
        <span class="item-title" title="${escapeHtml(it.title || '')}">${escapeHtml(title)}</span>
        <button class="restore-btn" onclick="restoreDeletion('${escapeHtml(String(numStr))}')"
                title="החזר את הפריט לגריד">↩ שחזר</button>
      </li>
    `;
  }).filter(Boolean).join('');
  list.innerHTML = html;
...
/* Apply deletions on initial load (so reload preserves the visual state) */
document.addEventListener('DOMContentLoaded', () => {
  try { applyDeletionsToCells(); } catch (e) {}
  try { updateDeletionsPanel(); } catch (e) {}
});
...
<div class="page">
  ...
  <div class="deletions-panel" id="deletionsPanel" hidden>
    <span class="deletions-panel-title">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
           stroke-linecap="round" stroke-linejoin="round">
        <polyline points="3 6 5 6 21 6"/>
        <path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/>
      </svg>
      פריטים שסומנו למחיקה (יוסרו בפרסום):
    </span>
    <ul class="deletions-list" id="deletionsList"></ul>
  </div>
  ''')}
