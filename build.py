...
THEME_TOKENS = {
    ...
    'uria': {
        ...
        'font': "'Plus Jakarta Sans', 'Rubik', sans-serif",
    },
}
...
/* Uria today: TEAL ring (the brand's secondary, distinct from holiday tangerine and post aubergine) */
body.theme-uria .cell.is-today {
  border-color: #0EA5BB !important;
  box-shadow: 0 0 0 2px #0EA5BB, 0 0 32px rgba(14,165,187,0.45) !important;
}
body.theme-uria .cell.is-today .cell-num { color: #0EA5BB !important; }
body.theme-uria .cell.is-today::after {
  background: #0EA5BB;
  color: #FFFFFF;
  box-shadow: 0 2px 8px rgba(14,165,187,0.4);
}
@keyframes uria-today-pulse {
  0%, 100% { box-shadow: 0 0 0 2px #0EA5BB, 0 0 24px rgba(14,165,187,0.4); }
  50% { box-shadow: 0 0 0 2px #0EA5BB, 0 0 40px rgba(14,165,187,0.7); }
}
...
/* Uria logo block - Logo 08b (two-block) - matching brand kit final-3.html exactly */
body.theme-uria .uria-x-logo {
  display: inline-flex;
  align-items: center;
  gap: 12px;
}
body.theme-uria .uria-x-logo .logo-two-block {
  display: inline-flex;
  align-items: stretch;
  height: 42px;
  box-shadow: 0 2px 8px rgba(76,29,149,0.15);
}
body.theme-uria .uria-x-logo .lb-name {
  background: #0EA5BB;
  color: #FAFAFA;
  display: flex;
  align-items: center;
...
body.theme-uria .uria-x-logo .lb-ub {
  background: #0A0A0A;
  color: #FAFAFA;
  display: flex;
  align-items: center;
...
body.theme-uria .uria-x-logo .logo-tag-strip {
  display: inline-flex;
  align-items: center;
  background: #0A0A0A;
  padding: 4px 10px;
...
body.theme-uria .uria-x-logo .logo-tag-strip-text {
  font-family: 'Plus Jakarta Sans', sans-serif;
  font-size: 9px;
  font-weight: 800;
...
body.theme-uria .uria-x-logo .logo-tag-strip-text .tag-accent { color: #FF6B35; }
body.theme-uria .uria-x-logo .x-block {
  display: flex;
  flex-direction: column;
  gap: 0;
}

/* Uria header strip - matching brand kit doc-strip-top pattern */
...
body.theme-uria .header::before {
  ...
  background: linear-gradient(90deg, #4C1D95 0%, #4C1D95 40%, #0EA5BB 70%, #FF6B35 100%);
}
...
/* Uria footer - doc-strip-bottom pattern */
...
/* Uria tabs - matching brand kit section-title style */
...
/* Type chips Uria - stronger, more brand-aligned */
...
/* Title text in cells - more contrast */
...
def render_html(data: dict, logo_b64: str, mode: str = 'zeliger') -> str:
    ...
    if mode == 'uria':
        # Logo 08b - two-block format from brand kit (final-3.html lines 1003-1011)
        logo_tag = (
            '<div class="uria-x-logo">'
            '<div class="x-block">'
            '<div class="logo-two-block">'
            '<div class="lb-name">אוריה ברמן</div>'
            '<div class="lb-ub">UB</div>'
            '</div>'
            '<div class="logo-tag-strip">'
            '<span class="logo-tag-strip-text">Strategy · Creative · Digital · <span class="tag-accent">AI</span></span>'
            '</div>'
            '</div>'
            '</div>'
        )
        footer_text = 'Built by <span class="footer-brand">אוריה ברמן</span><span class="footer-brand-mark">UB</span>'
    else:
