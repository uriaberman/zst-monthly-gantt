...
/* PENDING-DELETE state for cell-item (set by applyDeletionsToCells before publish).
   Visually clearer than archived: line-through + heavier dim + red tint, signaling
   that THIS WILL BE GONE after publish (vs. "נגנז" which says "still here, muted"). */
...
.cell:has(.cell-item.is-pending-delete)::after {
  content: 'למחיקה';
  position: absolute;
  top: 8px;
  left: 50%;
...
/* Show the draft bar whenever there are local changes (Editor only).
   Counts edits AND pending deletions (Iron Rule #20). */
...
window.requestDelete = function(num) {
  ...
  if (dels.includes(String(num))) {
    // Already marked - offer to undo
    if (confirm(`'${it.title}' כבר מסומן למחיקה.\n\nלבטל את הסימון?`)) {
      saveDeletions(dels.filter(d => String(d) !== String(num)));
  ...
  const msg =
    `למחוק לצמיתות את:\n\n"${it.title}"\n\n` +
    `הפריט יסומן למחיקה ויוסר מ-data.json בפרסום הבא.\n` +
    `עד הפרסום אפשר לבטל דרך 'בטל טיוטה' או דרך אותו כפתור סל בפריט.\n\n` +
    `להמשיך?`;
